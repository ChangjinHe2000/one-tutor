...
OneTutor 不绑定某一个考试。只要你能提供资料、笔记或题库，它就可以把内容整理成“出题 → 作答 → 诊断 → 复习”的闭环。下面先看真实使用界面，再用四六级英语和软考复习各举一个例子。

### 实际使用：从资料到当天练习

使用时不需要先手动整理项目结构。你只要告诉 Codex：资料在哪里、每次想练多少题、用什么语言、优先什么题型，OneTutor 就会在本地建立学习项目并生成当天练习。

<p align="center">
  <a href="assets/soft-exam-project-created.png">
    <img src="assets/soft-exam-project-created.png" alt="OneTutor 根据本地软考资料建立复习项目，导入有效题目，过滤异常题和重复题，并生成当天练习" width="760">
  </a>
</p>

<p align="center"><sub>建立学习项目后，OneTutor 会告诉你导入了多少有效题、过滤了多少异常题、去掉了多少重复题，以及当天练习卷在哪里。</sub></p>

这一步可以理解成把散落的资料变成一个可持续复习的本地项目：

| 画面里的结果 | 通俗解释 |
| --- | --- |
| `one-tutor-study` | 你的本地学习项目目录，题库、练习记录和反馈都会放在这里 |
| `217` 道有效题 | 可用于出题和复习的题目数量 |
| `11` 道异常/不计分题 | 题干缺失、答案不可靠或不适合直接计分的题会先排除 |
| `172` 道重复题 | 相同或高度重复的题会被去重，避免反复刷到同一题 |
| `2026-06-24.md` | 当天生成的练习卷，可以直接打开作答 |

打开生成的 Markdown 文件后，会看到一份不泄露答案的练习卷。每道题按题号展示知识点、题干和选项，做完后按 README 后面的格式把答案回给 Codex 即可。

<p align="center">
  <a href="assets/soft-exam-quiz-preview.png">
    <img src="assets/soft-exam-quiz-preview.png" alt="OneTutor 生成的软考自适应测验 Markdown 文件，按题号展示知识点、题干和选项" width="760">
  </a>
</p>

<p align="center"><sub>生成的练习卷只包含题目和选项；答案、解析和复习安排会在你提交答案后再生成。</sub></p>

截图里的本地路径只是示例。实际使用时，把路径换成你自己的资料目录即可。
